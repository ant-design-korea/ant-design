...
5.0 버전에서 테마에 영향을 미치는 가장 작은 엘리먼트를 **디자인 토큰**이라고 부릅니다. 디자인토큰을 수정하여, 다양한 테마나 컴포넌트를 구현할 수 있습니다. `ConfigProvider`에 `theme`를 전달하여 테마를 커스터마이징 할 수 있습니다. V5로 마이그레이션 한 후에는 V5테마가 기본적으로 적용될 것입니다.

<!-- prettier-ignore -->
:::warning
`ConfigProvider`는 `message.xxx`, `Modal.xxx`, `notification.xxx`,와 같은 정적 메서드에는 적용되지 않습니다. antd가 이러한 메서드들을 리액트 속성인 `ReactDOM.render`를 통해 동적으로 새로운 React엔티티로 생성하기 때문입니다. 이러한 컨텍스트는 현재 코드의 컨텍스트와 동일하지 않으므로 컨텍스트 정보를 얻을 수 없습니다.

<!-- prettier-ignore -->
컨텍스트 정보가 필요하다면 (ConfigProvider에 의해 구성된 컨텐트와 같은),  `Modal.useModal`메서드를 사용하여 모달 속성과 컨텍스트홀더 노드를 리턴 할 수 있습니다. 컨텍스트를 얻고자 하는 곳에 삽입하거나, 모달을 사용하는 것의 문제를 단순화 하기 위해 [앱 컴포넌트](/components/app)를 사용하거나, 수동으로 컨텍스트 홀더를 주입해야할 수도 있습니다.
:::
...
다양한 스타일의 테마는 `algorithm`을 수정함으로써 빠르게 생성될 수 있습니다. Ant Design 5.0은 세가지의 프리셋 알고리즘을 기본적으로 제공합니다:
...
기본적으로 모든 컴포넌트 토큰은 글로벌 토큰만 재정의할 수 있으며 시드 토큰으로부터 파생되지 않습니다.
...
v5에서 동적 테마 전환은 사용자에게 매우 간단합니다. `ConfigProvider`의 `theme` 속성을 통해 어떤 추가 설정 없이도 언제든지 테마를 전환할 수 있습니다.
...
`ConfigProvider`를 중첩하여 페이지의 특정 영역을 지역 테마로 적용할 수 있습니다. 자식 테마에서 변경되지 않은 디자인토큰은 부모 테마에서 상속될 것입니다.
...
현재 테마에서 디자인 토큰을 사용하려면 `useToken`훅을 통해 디자인 토큰 값을 가져올 수 있습니다.
...
리액트 라이프 사이클 외부에서 토큰이 필요하다면, 정적 함수를 사용하여 얻을 수 있습니다:
...
ConfigProvider와 동일하게 `getDesignToken`도 `theme`로 설정 객체를 받을 수 있습니다:
...
less와 같은 프리프로세스 스타일 프레임워크를 사용하고 싶다면, less-loader를 사용하여 주입하세요:
...
사용자가 테마 디버깅을 돕기 위해 도구를 제공하고 있습니다: [테마 에디터](/theme-editor)

원하는 테마에 맞게 이 도구를 사용하여 자유롭게 디자인 토큰을 수정할 수 있습니다.

## 디자인 토큰

디자인 토큰에서는 디자인에 더 적합한 3계층 구조를 제공하며, 이는 시드 토큰, 맵 토큰, 별칭 토큰으로 구성됩니다. 이 세 가지 토큰 세트는 단순한 그룹이 아닌 시드 토큰이 맵 토큰에서 파생되고, 맵 토큰이 별칭(Alias) 토큰에서 파생되는 3계층 파생 관계입니다. 대부분의 경우 시드 토큰은 테마 커스터마이징에 충분하지만, 더 높은 수준의 테마 커스터마이징이 필요한 경우 디자인 토큰의 라이프사이클을 이해해야 합니다.
...
시드 토큰은 모든 디자인 의도의 근원을 의미합니다. 예를들어, `colorPrimary`를 변경함으로써 테마 색상을 변경할 수 있으며, antd 내부 알고리즘이 시드토큰을 기반으로 연관된 색상들을 자동으로 계산하고 적용합니다:
